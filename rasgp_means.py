...
if __name__ == '__main__':
    ...
    for rep in range(reps):
        ...
        with open(save_path + file_name, 'wb') as f:
            pickle.dump({'E_opts': E_opts, 'E2_opts': E2_opts, 'E_cons': E_cons,
                         'rep_all': rep_all, 'params': params, 'g': g}, f)

        print 'data saved! total # of simulations: %d' % rep_all

        E_opt_avg = np.median(E_opts, axis=0)
        E2_opt_avg = np.median(E2_opts, axis=0)
        draw_fig(E_opt_avg, E2_opt_avg, rep_all)

        plt.plot(range(len(E_opt_avg)), range(len(E_opt_avg)) * E_opt_avg * step, label=r'$k \times E$')
        h = lu * d * (n) * (ns ** 2) / (12 * (mu ** 2))
        plt.axhline(h, linewidth=1.5, linestyle=(10, (5, 5)), c='g', label=r'$L_u \sigma^2 /\mu^2$')
        plt.axis([0, len(E_opt_avg), 0, max(E_opt_avg[-1] * len(E_opt_avg) * step * 2, 1.5 * h)])
        plt.show()
        # time.sleep(5)
